...
from src.services.product_service import add_product, get_product_by_name, delete_product, update_product, get_products, \
    searching_products
...
async def get_by_name(product: ProductCreate, db: AsyncSession = Depends(get_async_session),
                         current_user: User = Depends(get_current_user)):
    return await get_product_by_name(db, product.name, current_user.id)
